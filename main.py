...
#            urls.update({ 'another url': 'http://fake.com' })
        ...
        if success_status == 'success':
            message = '''<p>Amazingly... it looks like it may have worked. Click <a href='/download/{0}'>here</a>, then check your downloads folder for a file named "calendar.ics". <b>Before importing to your primary calendar, I highly recommend you either inspect it in a text editor and/or import it into a junk calendar.</b></p><p>While I can\'t think of a way this could lead to data loss, I\'m sure that it\'s possible, and it could most certainly lead to a buch of junk calendar events that would be a pain to (manually) clean out. So create a new calendar and import it there, just to be safe. If it works, delete that junk calendar and import to your real one.</p><p>Hope you\'ve found it helpful!</p>'''.format(blob_key)
...
#            urls.update({ 'another url': 'http://fake.com' })
        ...
        if success_status == 'failure':
            ...
            'error2': 'the headers looked okay, but something went wrong in the actual file conversion. Makes sure it was a .csv file and not some other type of spreasheet format.',
            'error3': 'there was some kind of problem finalizing the file.',
            }

            message = '''<p>Well, something didn't work right.</p><p>If I had to guess, I would say {0}</p><p>I <em>highly</em> recommend you read the instructional post at least once.</p>'''.format(error_dict[blob_key])
...
class UploadHandler(webapp2.RequestHandler):
    def post(self):
        ...
        csvFile = list(csv.DictReader(StringIO(
        self.request.get('csvUpload',
        default_value='')), skipinitialspace = True))
        ...
        if icsFile[0:5] == 'error':
            ...
            # Finalize the file. Do this before
